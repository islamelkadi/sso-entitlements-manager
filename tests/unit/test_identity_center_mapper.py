...
def test_list_identity_center_entities(setup_mock_aws_environment: pytest.fixture, excluded_sso_users: List[str], excluded_sso_groups: List[str], excluded_permission_sets: List[str]) -> None:
    """
    ...
    match the expected number from setup_mock_aws_environment.
    """
    ...
    permission_sets_via_definitions = {permission_set_name: permission_set_id for permission_set_name, permission_set_id in setup_mock_aws_environment["sso_permission_set_name_id_map"].items() if permission_set_name not in excluded_permission_sets}
